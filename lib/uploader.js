...
/**
 * Telegra.ph වෙත ගොනු උඩුගත කිරීම
 * @param {String} Path 
 */
function TelegraPh (Path) {
	return new Promise (async (resolve, reject) => {
		if (!fs.existsSync(Path)) return reject(new Error("ගොනුව සොයාගත නොහැක"))
		try {
...
/**
 * WebP ස්ටිකර් වීඩියෝ (MP4) බවට පත් කිරීම
 * @param {String} path 
 */
	...
	return new Promise((resolve, reject) => {
		 ...
		 axios({
			  ...
			  axios({
				   ...
				   resolve({
						status: true,
						message: "නිර්මාණය කළේ NIMESHA MADHUSHAN",
						result: result
				   })
			  }).catch(reject)
		 }).catch(reject)
