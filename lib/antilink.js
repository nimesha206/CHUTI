...
async function Antilink(msg, sock) {
	...
	try {
		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} මෙහි ලින්ක් (Links) එවීම තහනම්\`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} ලින්ක් එවූ බැවින් ඉවත් කරන ලදී\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} අවවාද ${WARN_COUNT} කට පසු ඉවත් කරන ලදී\`\`\``,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} ලින්ක් එවූ බැවින් අවවාදයයි ${warningCount}/${WARN_COUNT}\`\`\``,
						mentions: [sender]
					});
				}
				break;
		...
		console.error('Antilink හි දෝෂයකි:', error);
	}
}
