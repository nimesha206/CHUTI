/**
 * CHUTI WA BOT - ATTP Sticker Generator
...
async function attpCommand(sock, chatId, message) {
    ...
    if (!text) {
        await sock.sendMessage(chatId, { text: '⚠️ කරුණාකර .attp විධානයෙන් පසුව අවශ්‍ය වචනය ඇතුළත් කරන්න.' }, { quoted: message });
        return;
    }

    try {
        const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);
        // Sticker Pack Name changed to CHUTI WA BOT
        const webpPath = await writeExifVid(mp4Buffer, { packname: 'ᴄʜᴜᴛɪ ᴡᴀ ʙᴏᴛ' });
        ...
        await sock.sendMessage(chatId, { text: '❌ ස්ටිකරය නිර්මාණය කිරීම අසාර්ථක විය.' }, { quoted: message });
    }
}
