...
async function deleteCommand(sock, chatId, message, senderId) {
    try {
        ...
        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'delete විධානය ඇඩ්මින් වරුන්ට පමනක් වලංගු වේ.' }, { quoted: message });
            return;
        }
        ...
        if (repliedParticipant && ctxInfo.stanzaId) {
            ...
            await sock.sendMessage(chatId, { text: 'කරුණාකර පරිශීලකයා reply කරන්න\'පණිවිඩ ඉවත් කිරීමට පරිශීලාකයා හෝ පණිවිඩය තෝරන්න.' }, { quoted: message });
            return;
        }
        ...
        if (toDelete.length === 0) {
            await sock.sendMessage(chatId, { text: 'එම පරිශීලකයාගේ පණිවිඩ හමු නොවුනි.' }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: `ඉවත් කරන ලදි. ${toDelete.length} පණිවිඩය. පරිශීලක @${(targetUser||'').split('@')[0]}`, mentions: [targetUser] }, { quoted: message });
    } catch (err) {
        await sock.sendMessage(chatId, { text: 'පණිවිඩය ඉවත් කිරීමට අසමත් උණි.' }, { quoted: message });
    }
}
