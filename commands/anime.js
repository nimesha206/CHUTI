...
/**
 * CHUTI WA BOT - Anime Command Support
...
async function sendAnimu(sock, chatId, message, type) {
    ...
    async function convertMediaToSticker(mediaBuffer, isAnimated) {
        ...
        const json = {
            ...
            'sticker-pack-name': 'ᴄʜᴜᴛɪ ᴡᴀ ʙᴏᴛ',
            'sticker-pack-publisher': 'ɴɪᴍᴇsʜᴀ ᴍᴀᴅʜᴜsʜᴀන්',
            'emojis': ['🎌']
        };
    ...
    if (data.link) {
        ...
        try {
            await sock.sendMessage(
                chatId,
                { image: { url: link }, caption: `ᴀɴɪᴍᴇ: ${type}` },
                { quoted: message }
            );
            return;
        } catch {}
    ...
    await sock.sendMessage(
        chatId,
        { text: '❌ anime දත්ත ලබා ගැනීම අසාර්ථක විය.' },
        { quoted: message }
    );
}
    ...
    try {
        if (!sub) {
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: `භාවිතය: .animu <වර්ගය>\nවර්ග: ${apiTypes}` }, { quoted: message });
            } catch {
                await sock.sendMessage(chatId, { text: `භාවිතය: .animu <වර්ගය>\nවර්ග: ${supported.join(', ')}` }, { quoted: message });
            }
            return;
        }

        if (!supported.includes(sub)) {
            await sock.sendMessage(chatId, { text: `❌ නොගැලපෙන වර්ගයකි: ${sub}. මේවායින් එකක් උත්සාහ කරන්න: ${supported.join(', ')}` }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: '❌ ඇනිමේ ලබා ගැනීමේදී දෝෂයක් සිදු විය.' }, { quoted: message });
    }
}
