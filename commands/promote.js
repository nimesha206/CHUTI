...
async function promoteCommand(sock, chatId, mentionedJids, message) {
    ...
    try {
        ...
        const promotionMessage = `╭══✦〔 *තනතුරු ලබා දීම* 〕✦═╮\n│ \n` +
            `│ 👥 *ඇඩ්මින් තනතුර ලබාගත්තේ${userToPromote.length > 1 ? 's' : ''}:*\n` +
            `│ ${usernames.map(name => `• ${name}`).join('\n')}\n│\n` +
            `│ 👑 *තනතුර ලබා දුන්නේ:* @${promoterJid.split('@')[0]}\n│\n` +
...
async function handlePromotionEvent(sock, groupId, participants, author) {
    try {
       /* console.log('Promotion Event Data:', {
        ...
        const promotionMessage = `╭══✦〔 *තනතුරු ලබා දීම* 〕✦═╮\n│\n` +
            `│ 👥 *ඇඩ්මින් තනතුර ලබාගත්තේ${participants.length > 1 ? 's' : ''}:*\n` +
            `│ ${promotedUsernames.map(name => `• ${name}`).join('\n')}\n│\n` +
            `│ 👑 *තනතුර ලබා දුන්නේ:* ${promotedBy}\n│\n` +
            `│ 📅 *දිනය:* ${new Date().toLocaleString()}\n│\n` +
            `╰═✦═✦═✦═✦═✦═✦═✦═✦═✦═╯`;
