...
async function facebookCommand(sock, chatId, message) {
    try {
        ...
        if (!url.includes('facebook.com')) {
            return await sock.sendMessage(chatId, { 
                text: "මෙය ෆේස්බුක් ලින්කුවක් නොවේ."
            }, { quoted: message });
        }
        ...
        if (!data || data.status !== 200 || !data.success || !data.result) {
            return await sock.sendMessage(chatId, { 
                text: 'api ප්‍රෂ්නයකි පසුව උත්සහ කරන්න!'
            }, { quoted: message });
        }
        ...
        if (!fbvid) {
            return await sock.sendMessage(chatId, { 
                text: 'වැරදි ඩේටා. පසුව උත්සහ කරන්න'
            }, { quoted: message });
        }
        ...
        await sock.sendMessage(chatId, { 
            text: "api වලංගු නැත. බා ගැනීම අසාර්ථකයි: " + error.message
        }, { quoted: message });
    }
}
