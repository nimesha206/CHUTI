...
async function hideTagCommand(sock, chatId, senderId, messageText, replyMessage, message) {
    ...
    if (!isBotAdmin) {
        await sock.sendMessage(chatId, { text: 'කරුණාකර ඇඩ්මින් තනතුර ලබා දෙන්න.' }, { quoted: message });
        return;
    }

    if (!isSenderAdmin) {
        await sock.sendMessage(chatId, { text: 'ඇඩ්මින් වරුන්ට පමණයි එය භාවිතා කල හැක්කේ.' }, { quoted: message });
        return;
    }
