...
async function igsCommand(sock, chatId, message, crop = false) {
    try {
        ...
        if (!downloadData || !downloadData.data) {
            await sock.sendMessage(chatId, { text: '❌ ලින්කුවෙන් බා ගැනීම අසාර්ථකයි.' }, { quoted: message });
            return;
        }
        // Raw items
        ...
        if (items.length === 0) {
            await sock.sendMessage(chatId, { text: '❌ එම ලින්කුවේ කිසිවක් නැත.' }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: 'ලින්කුවෙන් ස්ටිකර් සෑදීම අසාර්ථකයි.' }, { quoted: message });
    }
}
...
async function forceMiniSticker(inputBuffer, isVideo, cropSquare) {
    ...
    const json = {
        'sticker-pack-id': crypto.randomBytes(32).toString('hex'),
        'sticker-pack-name': settings.packname || 'chuti_bot',
        'emojis': ['📸']
    };
    const exifAttr = Buffer.from([0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01, 0x00, 0x41, 0x57, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00]);
    const jsonBuffer = Buffer.from(JSON.stringify(json), 'utf8');
